# experiment_cpu.py
# CPU-only version of experiment_gpu.py.
...
NUM_WORKERS = 0
ARQUIVO_RESULTADOS_TXT = BASE_DIR / "training_results_cpu.txt"
ARQUIVO_RESULTADOS_CSV = BASE_DIR / "training_results_cpu.csv"
...
print(f"Dispositivo: {device}  (threads torch: {torch.get_num_threads()})")
...
def salvar_resultados(scenario, epoch, tempo_total, throughput, tempo_computacao, tempo_espera_dados, taxa_io_wait):
    linhas = [
        f"--- Results da Epoch {epoch} ({scenario}) ---",
        f"Total Epoch Time: {tempo_total:.2f} s",
        f"Throughput: {throughput:.2f} MB/s",
        f"Compute time on CPU: {tempo_computacao:.2f} s",
        f"Time waiting for data (I/O): {tempo_espera_dados:.2f} s",
        f"I/O wait rate: {taxa_io_wait:.1f}%",
        "-" * 30,
    ]
    ...
    with ARQUIVO_RESULTADOS_CSV.open("a", newline="", encoding="utf-8") as arquivo_csv:
        ...
        writer.writerow({
            ...
            "gpu_starvation_percent": f"{taxa_io_wait:.1f}",
        })
